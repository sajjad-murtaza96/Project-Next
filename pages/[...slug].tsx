...
const eventDetailPage = (props: StaticProps) => {
    ...
    const handleAddProductToCart = async () => {
        ...
        const [isSameProduct]: IProductData[] = products.map((item: IProductData) => {
    ...
    return (
        <>
            ...
            <div>
                <input onChange={(e) => { handleOnChangeQuantity(e); }} value={quantity} type="number" id='quantity' name='quantity' min='0' max='10' />
            </div>
            <button>
...
export const getStaticPaths = async () => {
    const data: { products: IProductData[] } = await getAllProducts();
    const pathWithParams = data.products.map(() => {
        return { params: { slug: [``] } }
    });
